export default function () {
  ...
  while (true) {
    try {
      user_name = prompt('토론방에 접속할 이름을 적어주세요.').trim();
  ...
  appendMessage({ message: `${user_name} 님이 입장하였습니다.` }, 'info');
  ...
  socket.on('user-connected', (uname) => {
    appendMessage({ message: `${uname} 님이 입장하였습니다.` }, 'info');
  });

  socket.on('user-disconnected', (uname) => {
    appendMessage({ message: `${uname} 님이 나갔습니다.` }, 'info');
  });
